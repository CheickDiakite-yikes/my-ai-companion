...
import {
  ...
  looksLikeGoogleActionCasualPauseTurn,
...
async function buildLiveMemoryContext(params: {
  ...
  suppressStructuredGoogleActionMemory?: boolean;
  ...
  const activeHistory: MemorySourceMessage[] = recentMessages
    ...
    .filter(
      (message) =>
        !(
          params.suppressStructuredGoogleActionMemory &&
          shouldExcludeMessageFromCompanionCurrentThreadDuringGooglePause({
            message,
  ...
  const pausedTaskCompanionHintLines: string[] = [];

  if (!params.suppressStructuredGoogleActionMemory) {
    ...
    for (const rawLine of buildPausedGoogleActionCompanionHint(googleConversationState)) {
      const sanitized = sanitizeMemoryText(rawLine, params.memoryPolicy);
      redactionCount += sanitized.redactionCount;
      if (!sanitized.text) continue;
      pausedTaskCompanionHintLines.push(sanitized.text);
    }
  }
    ...
    if (ENABLE_MEMORY_CONTEXT_QUERY_FUSION_V2) {
      try {
        const fusedContext = await retrieveFusedMemoryContext({
          ...
          googleHints: params.suppressStructuredGoogleActionMemory
            ? null
            : buildStructuredGoogleActionMemoryHints(googleConversationState),
        });
  ...
  if (pausedTaskCompanionHintLines.length > 0) {
    sections.push(
      ["Current Turn Guardrail:", ...pausedTaskCompanionHintLines].join("\n"),
    );
  }
    ...
    const runMemoryStage = async (
      ...
      return withTimeout(
        buildLiveMemoryContext({
          ...
          suppressStructuredGoogleActionMemory:
            params.suppressStructuredGoogleActionMemory ?? false,
        }),
        remainingMs,
...
function shouldExcludeMessageFromCompanionCurrentThreadDuringGooglePause(params: {
  message: {
    sender: string;
    text?: string | null;
    uiPayload?: unknown;
  ...
  googleConversationState: GoogleConversationState | null;
}): boolean {
  if (!params.googleConversationState) return false;
  const text = normalizeGoogleActionControlText(params.message.text ?? "");
  if (!text) return false;

  if (params.message.sender === "user") {
    return (
      detectGoogleActionTaskIntent(
        text,
        toGoogleRecentActionContext(params.googleConversationState),
      ) ||
      isGoogleActionSendMessage(text) ||
      isGoogleActionSaveAsDraftMessage(text) ||
      isGoogleActionApproveMessage(text) ||
      isGoogleActionDeclineMessage(text) ||
      looksLikeGoogleEmailDraftRevisionInstruction(text) ||
      looksLikeGoogleCalendarFollowUpInstruction(text) ||
      looksLikeGoogleEmailAmbiguitySelectionText(text) ||
      looksLikeGoogleCalendarAmbiguitySelectionText(text)
    );
  }

  if (params.message.sender === "assistant") {
    if (isAgentMessageUiPayload(params.message.uiPayload)) {
      return true;
    }
    return /\b(?:email|draft|recipient|subject|send|saved|approval|calendar|event|meeting|appointment)\b/i.test(
      text,
    );
  }
...
type GoogleConversationRecentTask = {
  taskId: string;
...
type GoogleClientActionContext = z.infer<typeof googleClientActionContextSchema>;

type GoogleConversationActionAmbiguity = {
  messageId: string;
  prompt: GoogleActionAmbiguityPrompt;
};

type GoogleConversationTaskTarget =
  | {
      connector: "gmail" | "calendar";
      source: "pending";
      taskId: string;
...
type GoogleConversationState = {
  ...
  foregroundEmailTaskId: string | null;
  foregroundCalendarTaskId: string | null;
  emailDraftCandidates: GoogleEmailConversationTaskTarget[];
  calendarEventCandidates: GoogleConversationTaskTarget[];
...
function isGoogleActionAmbiguityCandidatePayload(
  value: unknown,
): value is GoogleActionAmbiguityCandidate {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  ...
  return (
    ...
    (candidate.connector === "gmail" || candidate.connector === "calendar") &&
...
function isGoogleActionAmbiguityPromptPayload(
  value: unknown,
): value is GoogleActionAmbiguityPrompt {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  ...
  return (
    ...
    prompt.candidates.every((candidate) =>
      isGoogleActionAmbiguityCandidatePayload(candidate),
    )
  );
}
...
function getGoogleContextMetadataFromPayload(
  payload: AgentMessageUiPayload,
...
function isRecentGoogleEmailTask(
  preview: GoogleActionPreview | null,
  result: GoogleActionResult | null,
  ...
  return Boolean(
    preview?.connector === "gmail" &&
      ...
      (result?.status === "draft_created" || result?.status === "email_sent"),
  );
}
...
function isRecentGoogleCalendarTask(
  ...
  return Boolean(
    preview?.connector === "calendar" &&
      ...
      (result?.status === "event_created" || result?.status === "event_updated"),
  );
}

function isActionableRecentGoogleCalendarTask(
  preview: GoogleActionPreview | null,
  result: GoogleActionResult | null,
): preview is GoogleActionPreview {
  return Boolean(
...
function toGoogleRecentActionContext(
  state: GoogleConversationState,
): GoogleRecentActionContext {
  return {
    recentEmailTask: state.recentEmailTask,
    recentCalendarTask: state.recentCalendarTask,
  };
}

function isGoogleEmailDraftPreview(
...
function formatGoogleCalendarCandidateDateRange(
...
function formatGoogleCalendarCandidateTitle(
  candidate: GoogleConversationTaskTarget,
): string {
  return (
    candidate.preview.proposedCalendar?.title?.trim() ||
    candidate.preview.calendarEvent?.title?.trim() ||
    "Calendar event"
  );
}

function formatGoogleCalendarCandidateSubtitle(
  candidate: GoogleConversationTaskTarget,
): string | null {
  const startTime =
    candidate.preview.proposedCalendar?.startTime ??
    candidate.preview.calendarEvent?.startTime ??
    null;
  const endTime =
    candidate.preview.proposedCalendar?.endTime ??
    candidate.preview.calendarEvent?.endTime ??
    null;
  if (!startTime) return null;
  return formatGoogleCalendarCandidateDateRange(startTime, endTime);
}

function formatGoogleCalendarCandidateDetail(
  candidate: GoogleConversationTaskTarget,
): string | null {
  const location =
    candidate.preview.proposedCalendar?.location ??
    candidate.preview.calendarEvent?.location ??
    null;
  const description =
    candidate.preview.proposedCalendar?.descriptionPreview ??
    candidate.preview.calendarEvent?.description ??
    null;
  const detail = location?.trim() || description?.trim() || candidate.preview.summary.trim();
  return detail ? detail.replace(/\s+/g, " ").slice(0, 160) : null;
}
...
function buildGoogleCalendarCandidateStatusLabel(
  candidate: GoogleConversationTaskTarget,
): string {
  if (candidate.source === "pending") {
    return "Needs approval";
  }
  return candidate.preview.kind === "calendar_update" ? "Event updated" : "Event created";
}

function buildGoogleCalendarAmbiguityCandidateFromTarget(
  candidate: GoogleConversationTaskTarget,
  index: number,
): GoogleActionAmbiguityCandidate {
  const title = formatGoogleCalendarCandidateTitle(candidate);
  const subtitle = formatGoogleCalendarCandidateSubtitle(candidate);
  const ordinalHint = buildGoogleEmailAmbiguityOrdinalHint(index);
  return {
    taskId: candidate.taskId,
    connector: "calendar",
    title,
    subtitle,
    detail: formatGoogleCalendarCandidateDetail(candidate),
    statusLabel: buildGoogleCalendarCandidateStatusLabel(candidate),
    selectionPrompt: subtitle
      ? `Use the ${ordinalHint} calendar event ${title} at ${subtitle}.`
      : `Use the ${ordinalHint} calendar event ${title}.`,
  };
}
...
function resolveRelativeDayOffsetFromText(text: string): 0 | 1 | null {
  const normalized = normalizeGoogleActionControlText(text);
  if (!normalized) return null;
  if (/\btoday\b/i.test(normalized)) return 0;
  if (/\byesterday\b/i.test(normalized)) return 1;
  return null;
}

function isCandidateWithinRelativeDayOffset(
  createdAt: Date | null,
  offsetDays: 0 | 1,
): boolean {
...
function resolveGoogleEmailDraftCandidateFromText(params: {
  ...
  const ordinalMatchers: Array<{ pattern: RegExp; index: number }> = [
    {
      pattern:
        /\b(?:latest|newest|most recent|current|last one|last draft|last email|this one|this draft|this email|latest draft|latest email|latest message|first one|first draft|first email|first message|top one|top draft|top email)\b/i,
      index: 0,
    },
    {
      pattern:
        /\b(?:older|previous|older one|other one|other draft|second one|second draft|second email|second message|older draft)\b/i,
      index: 1,
    },
    { pattern: /\bthird(?:\s+(?:one|draft|email|message))?\b/i, index: 2 },
    { pattern: /\bfourth(?:\s+(?:one|draft|email|message))?\b/i, index: 3 },
  ];
  ...
  const relativeDayOffset = resolveRelativeDayOffsetFromText(normalized);
  if (relativeDayOffset !== null) {
    const dateMatches = candidates.filter((candidate) =>
      isCandidateWithinRelativeDayOffset(candidate.createdAt, relativeDayOffset),
    );
    if (dateMatches.length === 1) {
      return { kind: "resolved", candidate: dateMatches[0] };
    }
    if (dateMatches.length > 1) {
      return { kind: "ambiguous" };
    }
...
function dedupeGoogleConversationTaskTargets(
  candidates: GoogleConversationTaskTarget[],
): GoogleConversationTaskTarget[] {
  const seen = new Set<string>();
  const deduped: GoogleConversationTaskTarget[] = [];
  for (const candidate of candidates) {
    if (seen.has(candidate.taskId)) continue;
    seen.add(candidate.taskId);
    deduped.push(candidate);
...
function normalizeGoogleCalendarCandidateMatchText(value: string): string {
  return normalizeGoogleActionControlText(value)
    .replace(/[^a-z0-9@.\s:-]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
function resolveGoogleCalendarTaskCandidateFromText(params: {
  text: string;
  candidates: GoogleConversationTaskTarget[];
}):
  | {
  ...
  const candidates = dedupeGoogleConversationTaskTargets(params.candidates)
    .filter((candidate) => candidate.connector === "calendar")
    .slice(0, 4);
  if (candidates.length === 0) {
    return { kind: "none" };
  }
  ...
  const normalized = normalizeGoogleCalendarCandidateMatchText(params.text);
  if (!normalized) {
    return { kind: "ambiguous" };
  }
  ...
  const ordinalMatchers: Array<{ pattern: RegExp; index: number }> = [
    {
      pattern:
        /\b(?:latest|newest|most recent|current|last one|last event|last meeting|this one|this event|this meeting|latest event|latest meeting|first one|first event|first meeting|top one|top event|top meeting)\b/i,
      index: 0,
    },
    {
      pattern:
        /\b(?:older|previous|older one|other one|other event|other meeting|second one|second event|second meeting)\b/i,
      index: 1,
    },
    { pattern: /\bthird(?:\s+(?:one|event|meeting))?\b/i, index: 2 },
    { pattern: /\bfourth(?:\s+(?:one|event|meeting))?\b/i, index: 3 },
  ];
  for (const matcher of ordinalMatchers) {
    if (matcher.pattern.test(normalized) && candidates[matcher.index]) {
      return { kind: "resolved", candidate: candidates[matcher.index] };
  ...
  const pendingMatches = /\bpending|needs approval|preview\b/i.test(normalized)
    ? candidates.filter((candidate) => candidate.source === "pending")
    : [];
  if (pendingMatches.length === 1) {
    return { kind: "resolved", candidate: pendingMatches[0] };
  }
  if (pendingMatches.length > 1) {
    return { kind: "ambiguous" };
  }
  ...
  const titleMatches = candidates.filter((candidate) => {
    const title = normalizeGoogleCalendarCandidateMatchText(
      formatGoogleCalendarCandidateTitle(candidate),
    );
    return Boolean(title && normalized.includes(title));
  });
  if (titleMatches.length === 1) {
    return { kind: "resolved", candidate: titleMatches[0] };
  }
  if (titleMatches.length > 1) {
    return { kind: "ambiguous" };
  }

  const subtitleMatches = candidates.filter((candidate) => {
    const subtitle = normalizeGoogleCalendarCandidateMatchText(
      formatGoogleCalendarCandidateSubtitle(candidate) ?? "",
    );
    return Boolean(subtitle && normalized.includes(subtitle));
  });
  if (subtitleMatches.length === 1) {
    return { kind: "resolved", candidate: subtitleMatches[0] };
  }
  if (subtitleMatches.length > 1) {
    return { kind: "ambiguous" };
  }
...
function rankGoogleConversationTaskTargets<T extends GoogleConversationTaskTarget>(params: {
  candidates: T[];
  clientActionContext?: GoogleClientActionContext | null;
  connector: GoogleActionTargetConnector;
  action: "send" | "revise" | "update";
}): T[] {
  const activeTaskId = params.clientActionContext?.actionableTargetId ?? null;
  const activeConnector = params.clientActionContext?.connector ?? null;
  const prefersManualSelection =
    params.clientActionContext?.selectionReason === "manual_selection";
  const prefersForegroundTask =
    params.clientActionContext?.selectionReason === "foreground_task" ||
    params.clientActionContext?.selectionReason === "paused_task";
  return [...params.candidates].sort((a, b) => {
    const aActive = a.taskId === activeTaskId ? 1 : 0;
    const bActive = b.taskId === activeTaskId ? 1 : 0;
    if (aActive !== bActive) return bActive - aActive;
    const aConnector = a.connector === params.connector ? 1 : 0;
    const bConnector = b.connector === params.connector ? 1 : 0;
    if (aConnector !== bConnector) return bConnector - aConnector;
    const aContextConnector = a.connector === activeConnector ? 1 : 0;
    const bContextConnector = b.connector === activeConnector ? 1 : 0;
    if (aContextConnector !== bContextConnector) {
      return bContextConnector - aContextConnector;
    }
    if (a.messageIndex !== b.messageIndex) {
      return b.messageIndex - a.messageIndex;
    }
    if (prefersManualSelection && a.source !== b.source) {
      return a.source === "recent" ? -1 : 1;
    }
    if (prefersForegroundTask && a.source !== b.source) {
...
function resolveTargetFromClientActionContext<T extends GoogleConversationTaskTarget>(params: {
  candidates: T[];
  clientActionContext?: GoogleClientActionContext | null;
  connector: GoogleActionTargetConnector;
}): T | null {
  const targetTaskId = params.clientActionContext?.actionableTargetId?.trim() ?? "";
  if (!targetTaskId) {
    return null;
  }
  return (
    params.candidates.find(
      (candidate) =>
        candidate.connector === params.connector && candidate.taskId === targetTaskId,
    ) ?? null
  );
}

function hasExplicitAmbiguitySelectionContext(params: {
  clientActionContext?: GoogleClientActionContext | null;
  ...
  if (params.clientActionContext?.selectionReason !== "manual_selection") {
    return false;
  }
  if (params.clientActionContext?.connector !== params.ambiguity.connector) {
    return false;
  }
  const candidateIds = new Set(
    params.ambiguity.candidates.map((candidate) => candidate.taskId),
  );
  if (!candidateIds.has(targetTaskId)) {
    return false;
  }
  const hintedCandidateIds = params.clientActionContext?.candidateTargetIds ?? [];
  if (
    hintedCandidateIds.length > 0 &&
    hintedCandidateIds.some((candidateId) => !candidateIds.has(candidateId))
  ) {
    return false;
  }
  return true;
...
function looksLikeExplicitGoogleEmailTargetReferenceText(text: string): boolean {
  const normalized = normalizeGoogleEmailCandidateMatchText(text);
  if (!normalized) return false;
  return (
    Boolean(extractGoogleEmailAddressForMatching(normalized)) ||
    /\b(?:latest|newest|most recent|current|older|previous|other|first|second|third|fourth|that one|this one|subject|recipient)\b/i.test(
      normalized,
    ) ||
    /\b(?:draft|email|message|recipient|subject)\s+(?:from\s+)?(?:today|yesterday)\b/i.test(
      normalized,
    ) ||
    /\bthe\s+.+\s+from\s+(?:today|yesterday)\b/i.test(normalized) ||
    /\bthe\s+.+\s+one\b/i.test(normalized)
  );
}
...
function resolveForegroundGoogleEmailTaskTarget(
  state: GoogleConversationState,
  candidates: GoogleEmailConversationTaskTarget[],
): GoogleEmailConversationTaskTarget | null {
  if (state.pendingTask?.preview && isGoogleEmailDraftPreview(state.pendingTask.preview)) {
    return (
      candidates.find((candidate) => candidate.taskId === state.pendingTask?.taskId) ??
      null
    );
  }
  if (state.foregroundEmailTaskId) {
    const foregroundCandidate =
      candidates.find((candidate) => candidate.taskId === state.foregroundEmailTaskId) ??
      null;
...
function resolveForegroundGoogleCalendarTaskTarget(
  state: GoogleConversationState,
  candidates: GoogleConversationTaskTarget[],
): GoogleConversationTaskTarget | null {
  if (
    state.pendingTask?.preview?.connector === "calendar" &&
    state.pendingTask.preview.proposedCalendar
  ) {
    return (
      candidates.find((candidate) => candidate.taskId === state.pendingTask?.taskId) ??
      null
  ...
  if (state.foregroundCalendarTaskId) {
    const foregroundCandidate =
      candidates.find((candidate) => candidate.taskId === state.foregroundCalendarTaskId) ??
      null;
    if (foregroundCandidate) {
      return foregroundCandidate;
    }
...
function resolveLatestGoogleConversationState(messages: Message[]): GoogleConversationState {
  ...
  for (let idx = messages.length - 1; idx >= 0; idx -= 1) {
    ...
    const payloadGoogleContext = getGoogleContextMetadataFromPayload(payload);
    if (
      !state.foregroundEmailTaskId &&
      payloadGoogleContext?.connector === "gmail" &&
      typeof payloadGoogleContext.actionableTargetId === "string" &&
      payloadGoogleContext.actionableTargetId.trim().length > 0
    ) {
      state.foregroundEmailTaskId = payloadGoogleContext.actionableTargetId.trim();
    }
    if (
      !state.foregroundCalendarTaskId &&
      payloadGoogleContext?.connector === "calendar" &&
      typeof payloadGoogleContext.actionableTargetId === "string" &&
      payloadGoogleContext.actionableTargetId.trim().length > 0
    ) {
      state.foregroundCalendarTaskId = payloadGoogleContext.actionableTargetId.trim();
    }
    ...
    if (
      ...
      if (
        !state.actionAmbiguity &&
        !state.pendingTask &&
        state.emailDraftCandidates.length === 0 &&
        state.calendarEventCandidates.length === 0 &&
        !state.composeSession &&
        !state.calendarSession &&
        !hasNewerGoogleActionState &&
        isGoogleActionAmbiguityPromptPayload(payload.ambiguity)
      ) {
        state.actionAmbiguity = {
          messageId: message.id,
    ...
    if (payload.kind === "agent_approval") {
      ...
      if (
        ...
        isFreshGoogleTaskId &&
        payload.approval.status === "pending" &&
        !seenCalendarTaskIds.has(payload.taskId) &&
        preview.connector === "calendar" &&
        preview.proposedCalendar
      ) {
          state.calendarEventCandidates.push({
            connector: "calendar",
            source: "pending",
            taskId: payload.taskId,
            preview,
        ...
        seenCalendarTaskIds.add(payload.taskId);
      }
      continue;
    }
    ...
    if (payload.kind === "agent_task_status") {
      ...
      seenGoogleTaskIds.add(payload.task.id);
      if (payload.task.status === "approval_required" && !state.pendingTask) {
        state.pendingTask = {
      ...
      if (payload.task.status === "completed" && preview) {
        if (!state.recentEmailTask && isRecentGoogleEmailTask(preview, result)) {
        ...
        if (
          ...
          isRecentGoogleCalendarTask(preview, result)
        ) {
        state.recentCalendarTask = {
          taskId: payload.task.id,
          preview,
        ...
        if (
          !seenCalendarTaskIds.has(payload.task.id) &&
          isActionableRecentGoogleCalendarTask(preview, result)
        ) {
          state.calendarEventCandidates.push({
            connector: "calendar",
            source: "recent",
          ...
          seenCalendarTaskIds.add(payload.task.id);
        }
      }
      continue;
    }
...
function buildPausedGoogleActionCompanionHint(
  state: GoogleConversationState | null,
): string[] {
  if (!state) return [];
  const hasActiveWorkflow = Boolean(
    state.pendingTask || state.composeSession || state.calendarSession || state.actionAmbiguity,
  );
  if (hasActiveWorkflow) {
    return [
      "- The current user turn is a short social/casual aside while a Gmail or Calendar workflow is paused.",
      "- Reply socially and naturally to the current turn.",
      "- Do not continue, summarize, or speculate about the paused Gmail/Calendar task unless the user explicitly asks to resume it.",
      "- Do not mention email addresses, subject lines, draft contents, or calendar details in this reply.",
    ];
  }
  if (state.recentEmailTask || state.recentCalendarTask) {
    return [
      "- The current user turn is a normal social/casual turn after a recent Gmail or Calendar action.",
      "- Reply to the present social turn only.",
      "- Do not drag the previous Gmail/Calendar action back into the conversation unless the user explicitly resumes or references it.",
      "- Do not mention email addresses, subject lines, draft contents, or calendar details in this reply.",
    ];
  }
  return [];
...
function shouldSuppressStructuredGoogleActionMemoryForCompanionTurn(params: {
  text: string;
  state: GoogleConversationState | null;
}): boolean {
  if (!params.state) return false;
  const hasRelevantGoogleState = Boolean(
    params.state.pendingTask ||
      params.state.composeSession ||
      params.state.calendarSession ||
      params.state.actionAmbiguity ||
      params.state.recentEmailTask ||
      params.state.recentCalendarTask,
  );
  if (!hasRelevantGoogleState) return false;
  return looksLikeGoogleActionCasualPauseTurn(params.text);
}

function normalizeGoogleActionControlText(text: string): string {
  return normalizeApostrophes(toCompactMessageText(text)).toLowerCase();
}
...
function isGoogleActionApproveMessage(text: string): boolean {
  ...
  return (
    compact.includes("looks good") ||
    compact.includes("sounds good") ||
    compact.includes("that's good") ||
    compact.includes("thats good") ||
    compact.includes("go ahead") ||
    /\bapprove(?:d)?\b/.test(compact)
  );
}

function isGoogleActionDeclineMessage(text: string): boolean {
  const compact = normalizeGoogleActionControlText(text);
  if (!compact) return false;
  ...
  return (
    ...
    compact.includes("never mind") ||
    compact.includes("nevermind") ||
    compact.includes("cancel it") ||
    compact.includes("stop that")
  );
}
...
function shouldBypassGenericAgentTaskForGoogleAction(params: {
  ...
  if (
    state.pendingTask?.preview?.connector === "gmail" &&
  ...
  if (
    ...
    !detectGoogleActionTaskIntent(params.text, toGoogleRecentActionContext(state)) &&
    looksLikeGoogleEmailDraftRevisionInstruction(params.text)
  ) {
    return true;
  }
...
function looksLikeGoogleComposeSubjectOnlyUpdateText(text: string): boolean {
  const compact = normalizeGoogleActionControlText(text);
  if (!compact) return false;
  if (!/\bsubject(?: line)?\b/.test(compact)) return false;
  if (/\b(?:body|message|say|tell|ask|recipient)\b/.test(compact)) return false;
  return true;
}
...
function buildGoogleTargetContextMetadata(params: {
  ...
  selectionMode?: GoogleActionTargetContextMetadata["selectionMode"];
}): GoogleActionTargetContextMetadata {
  return {
    connector: params.connector,
    action: params.action,
    actionableTargetId: params.actionableTargetId ?? null,
    candidateTargetIds: params.candidateTargetIds ?? [],
    sourceTurnId: params.sourceTurnId ?? null,
    selectionReason: params.selectionReason ?? null,
    surfaceKey: params.surfaceKey ?? null,
...
function toGoogleComposeSessionUiPayload(
  ...
  return {
    ...
    session,
    text,
    googleContext: googleContext ?? null,
  };
}
...
async function createGoogleComposeSessionAssistantMessage(params: {
  ...
  return params.storage.createMessage({
    ...
    text: params.text,
    partIndex: 0,
    uiPayload: toGoogleComposeSessionUiPayload(
      params.session,
      params.text,
...
async function createGoogleCalendarSessionAssistantMessage(params: {
  ...
  return params.storage.createMessage({
    ...
    text: params.text,
    partIndex: 0,
    uiPayload: toGoogleCalendarSessionUiPayload(
      params.session,
      params.text,
      params.googleContext,
    ),
...
function toGoogleActionAmbiguityUiPayload(
  ambiguity: GoogleActionAmbiguityPrompt,
  text: string,
  googleContext?: GoogleActionTargetContextMetadata | null,
): Extract<AgentMessageUiPayload, { kind: "agent_google_action_ambiguity" }> {
  return {
...
async function createGoogleActionAmbiguityAssistantMessage(params: {
  storage: typeof storage;
  conversationId: string;
  ambiguity: GoogleActionAmbiguityPrompt;
  text: string;
  ...
  return params.storage.createMessage({
    ...
    text: params.text,
    partIndex: 0,
    uiPayload: toGoogleActionAmbiguityUiPayload(
      params.ambiguity,
      params.text,
      params.googleContext,
    ),
...
function buildRecentGoogleActionTaskFromTarget(
  target: GoogleConversationTaskTarget,
): RecentGoogleActionTask {
  return {
    taskId: target.taskId,
    preview: target.preview,
    result: "result" in target ? target.result ?? null : null,
  };
}

function looksLikeGoogleCalendarFollowUpInstruction(text: string): boolean {
  const compact = normalizeGoogleActionControlText(text);
  if (!compact) return false;
  return (
    /\b(move|reschedule|change|update|add|set|clear|remove|book|put|block|hold)\b/.test(
      compact,
    ) &&
    (/\b(calendar|meeting|event|appointment)\b/.test(compact) ||
      /\b(it|that)\b/.test(compact) ||
      /\b(location|notes?|description)\b/.test(compact) ||
      /\bto\s+\d{1,2}(?::\d{2})?\s*(?:am|pm)\b/.test(compact))
  );
}

function looksLikeGoogleCalendarAmbiguitySelectionText(text: string): boolean {
  const normalized = normalizeGoogleCalendarCandidateMatchText(text);
  if (!normalized) return false;
  return /\b(?:latest|newest|most recent|current|older|previous|first|second|third|pending|created|updated|event|meeting|that one|this one)\b/i.test(
    normalized,
  );
}

function looksLikeExplicitGoogleCalendarTargetReferenceText(text: string): boolean {
  const normalized = normalizeGoogleCalendarCandidateMatchText(text);
  if (!normalized) return false;
  return (
    /\b(?:latest|newest|most recent|current|older|previous|first|second|third|pending|created|updated|event|meeting|appointment|that one|this one)\b/i.test(
      normalized,
    ) ||
    /\b(?:event|meeting|appointment)\s+(?:from\s+)?(?:today|yesterday)\b/i.test(
      normalized,
    ) ||
    /\bthe\s+.+\s+from\s+(?:today|yesterday)\b/i.test(normalized) ||
    /\bthe\s+.+\s+one\b/i.test(normalized)
...
async function handleResolvedGoogleEmailFollowUp(params: {
  ...
  if (params.action === "send") {
    ...
    const finalizedAssistantMessages =
      assistantMessages.length > 0
        ...
        : [
            {
              id: `google-action-send-followup-${run.task.id}`,
              conversationId: params.conversationId,
              sender: "assistant",
              turnId: randomUUID(),
              partIndex: 0,
              text: "I prepared a send-ready approval for that draft. Review it and approve when you're ready.",
              createdAt: new Date(),
              attachments: [],
              uiPayload: {
                kind: "agent_task_status",
                ...
                googleActionPreview: run.preview,
                googleContext: params.googleContext ?? null,
              },
            },
          ];
  ...
  if (task.status === "approval_required") {
    ...
    const finalizedAssistantMessages =
      assistantMessages.length > 0
        ...
        : [
            {
              id: `google-action-revision-pending-${revised.task.id}`,
              conversationId: params.conversationId,
              sender: "assistant",
              turnId: randomUUID(),
              partIndex: 0,
              ...
              uiPayload: {
                ...
                googleActionPreview: revised.preview,
                googleContext: params.googleContext ?? null,
              },
            },
          ];
  ...
  const finalizedAssistantMessages =
    assistantMessages.length > 0
      ...
      : [
          {
            id: `google-action-revision-followup-${run.task.id}`,
            conversationId: params.conversationId,
            sender: "assistant",
            turnId: randomUUID(),
            partIndex: 0,
...
async function finalizePreparedGoogleActionTask(params: {
  ...
  text: string;
  preparation: Awaited<ReturnType<typeof prepareGoogleActionTask>>;
  onEvent?: (event: AgentTaskEvent) => void;
  googleContext?: GoogleActionTargetContextMetadata | null;
}) {
  ...
  if (preparation.kind === "clarify" || preparation.kind === "upgrade_required") {
    const assistantMessages =
      preparation.kind === "clarify" && preparation.composeSession
        ? [
            await createGoogleComposeSessionAssistantMessage({
              storage: params.storage,
              conversationId: params.conversationId,
              session: preparation.composeSession,
              text: preparation.message,
              googleContext: params.googleContext,
            }),
          ]
  ...
  const run = await startGoogleActionTaskRun({
    ...
    userId: params.userId,
    conversationId: params.conversationId,
    prompt: preparation.resolvedPrompt ?? params.text,
...
async function maybeHandleGoogleActionTask(params: {
  ...
  text: string;
  clientTimeZone?: string | null;
  clientGoogleActionContext?: GoogleClientActionContext | null;
  userMessage: {
    id: string;
    createdAt: Date | null;
  };
  ...
  const rankedEmailCandidates: GoogleEmailConversationTaskTarget[] = rankGoogleConversationTaskTargets({
    candidates: googleConversationState.emailDraftCandidates,
    clientActionContext: params.clientGoogleActionContext,
    connector: "gmail",
    action: isGoogleActionSendMessage(params.text) ? "send" : "revise",
  });
  const foregroundEmailTaskTarget = resolveForegroundGoogleEmailTaskTarget(
    googleConversationState,
    rankedEmailCandidates,
  );
  const pendingEmailTaskTarget =
    googleConversationState.pendingTask?.preview &&
    isGoogleEmailDraftPreview(googleConversationState.pendingTask.preview)
      ? ({
          connector: "gmail",
          source: "pending",
          taskId: googleConversationState.pendingTask.taskId,
          preview: googleConversationState.pendingTask.preview,
          messageIndex: googleConversationState.pendingTask.messageIndex,
          createdAt: googleConversationState.pendingTask.createdAt,
        } satisfies GoogleEmailConversationTaskTarget)
      : null;
  const latestEmailTaskTarget =
    foregroundEmailTaskTarget ??
    (rankedEmailCandidates[0] as GoogleEmailConversationTaskTarget | undefined) ??
    resolveLatestGoogleEmailTaskTarget(googleConversationState);
  const rankedCalendarCandidates = rankGoogleConversationTaskTargets({
    candidates: googleConversationState.calendarEventCandidates,
    clientActionContext: params.clientGoogleActionContext,
    connector: "calendar",
    action: "update",
  });
  const foregroundCalendarTaskTarget = resolveForegroundGoogleCalendarTaskTarget(
    googleConversationState,
    rankedCalendarCandidates,
  );
  ...
  const actionableEmailCandidates = rankedEmailCandidates;
  ...
  const calendarCandidateResolution = resolveGoogleCalendarTaskCandidateFromText({
    text: params.text,
  ...
  const explicitContextSelectedCalendarTarget = hasExplicitTargetSelection
    ...
    : null;
  const activeContextSelectedEmailTarget = hasExplicitTargetSelection
    ? null
    : contextSelectedEmailTarget;
  const activeContextSelectedCalendarTarget = hasExplicitTargetSelection
    ? null
    : contextSelectedCalendarTarget;
  const hasForegroundEmailTarget = Boolean(foregroundEmailTaskTarget);
  const hasForegroundCalendarTarget = Boolean(foregroundCalendarTaskTarget);
  const hasResolvableEmailForeground = Boolean(
    foregroundEmailTaskTarget || activeContextSelectedEmailTarget,
  );
  const hasResolvableCalendarForeground = Boolean(
    foregroundCalendarTaskTarget || activeContextSelectedCalendarTarget,
  );
  const hasExplicitEmailTargetReference = looksLikeExplicitGoogleEmailTargetReferenceText(
    params.text,
  );
  const hasExplicitCalendarTargetReference =
    looksLikeExplicitGoogleCalendarTargetReferenceText(params.text);
  ...
  const hasActiveGoogleWorkflow = Boolean(
    googleConversationState.pendingTask ||
      hasActiveComposeSession ||
      hasActiveCalendarSession ||
      googleConversationState.actionAmbiguity,
  );
  const shouldPauseForCasualTurn =
    hasActiveGoogleWorkflow &&
    !startsFreshGoogleActionRequest &&
    looksLikeGoogleActionCasualPauseTurn(params.text);
  const looksLikeEmailAmbiguityReply =
    looksLikeGoogleEmailAmbiguitySelectionText(params.text) ||
    isGoogleActionSendMessage(params.text) ||
    looksLikeGoogleEmailDraftRevisionInstruction(params.text);
  const looksLikeCalendarAmbiguityReply =
    looksLikeGoogleCalendarAmbiguitySelectionText(params.text) ||
    looksLikeGoogleCalendarFollowUpInstruction(params.text);
  const prefersPausedForegroundEmailTarget =
    params.clientGoogleActionContext?.selectionReason === "paused_task";
  const canImplicitlyResumePendingEmailTask =
    Boolean(pendingEmailTaskTarget) &&
    !startsFreshGoogleActionRequest &&
    !hasExplicitEmailTargetReference &&
    !explicitContextSelectedEmailTarget &&
    emailCandidateResolution.kind !== "resolved";

  params.onTrace?.("google.target_resolution", {
    text: params.text,
    ...
    calendarCandidateTaskIds: rankedCalendarCandidates.map((candidate) => candidate.taskId),
    ...
    activeCalendarSessionStatus: hasActiveCalendarSession
      ...
      : null,
    foregroundEmailTaskId: foregroundEmailTaskTarget?.taskId ?? null,
    foregroundCalendarTaskId: foregroundCalendarTaskTarget?.taskId ?? null,
    shouldPauseForCasualTurn,
  });

  if (shouldPauseForCasualTurn) {
    params.onTrace?.("google.task_paused_by_casual_turn", {
      text: params.text,
      hasPendingTask: Boolean(googleConversationState.pendingTask),
  ...
  if (googleConversationState.actionAmbiguity) {
    ...
    if (
      ...
      params.onTrace?.("google.target_ambiguity_context_ignored", {
        connector: googleConversationState.actionAmbiguity.prompt.connector,
    ...
    if (shouldBypassAmbiguityForActiveSession) {
      ...
      const selectedEmailTarget =
        ...
        (emailCandidateResolution.kind === "resolved"
          ...
          : null);
      if (!selectedEmailTarget) {
        return { handled: false as const };
      }
      params.onTrace?.("google.target_ambiguity_resolved", {
        connector: "gmail",
        action: googleConversationState.actionAmbiguity.prompt.action,
      ...
      const googleContext = buildGoogleTargetContextMetadata({
        ...
        selectionMode: params.clientGoogleActionContext?.selectionMode ?? "manual",
      });
      return handleResolvedGoogleEmailFollowUp({
        ...
        targetPreview: selectedEmailTarget.preview,
        googleContext,
        onEvent: params.onEvent,
      });
    }
    ...
    if (
      ...
      const googleContext = buildGoogleTargetContextMetadata({
        connector: "calendar",
        action: googleConversationState.actionAmbiguity.prompt.action,
        actionableTargetId: selectedTarget.taskId,
      ...
      const preparation = await prepareGoogleActionTask({
        storage: params.storage,
        userId: params.userId,
        text: googleConversationState.actionAmbiguity.prompt.instructionText,
        clientTimeZone: params.clientTimeZone ?? null,
        recentContext: {
          ...recentContext,
          recentCalendarTask: buildRecentGoogleActionTaskFromTarget(selectedTarget),
        },
      });
      return finalizePreparedGoogleActionTask({
        storage: params.storage,
    ...
    if (
      ...
      const connector = googleConversationState.actionAmbiguity.prompt.connector;
      const assistantMessage = await createGoogleActionAmbiguityAssistantMessage({
        storage: params.storage,
        conversationId: params.conversationId,
        ambiguity: googleConversationState.actionAmbiguity.prompt,
        text:
          connector === "gmail"
            ? "I found more than one recent draft. Which email did you mean?"
            : "I found more than one recent calendar event. Which one did you mean?",
        googleContext: buildGoogleTargetContextMetadata({
          connector,
          action: googleConversationState.actionAmbiguity.prompt.action,
          candidateTargetIds:
      ...
      return {
        ...
        model:
          connector === "gmail"
            ? "google_action_email_ambiguity_v1"
            : "google_action_calendar_ambiguity_v1",
        decisionPath: "companion_reply" as const,
        decisionPathReason: "companion" as const,
        awaitingApproval: false,
        task: null,
  ...
  if (googleConversationState.composeSession) {
    const composeSessionGoogleContext = buildGoogleTargetContextMetadata({
      connector: "gmail",
      action: "create",
      actionableTargetId:
        params.clientGoogleActionContext?.connector === "gmail"
      ...
      sourceTurnId: params.userMessage.id,
      selectionReason:
        params.clientGoogleActionContext?.selectionReason ?? "clarification_session",
      surfaceKey: params.clientGoogleActionContext?.surfaceKey ?? null,
    ...
    if (looksLikeGoogleClarificationConfusionText(params.text)) {
      ...
      const assistantMessage = await createGoogleComposeSessionAssistantMessage({
        ...
        googleContext: composeSessionGoogleContext,
      });
      const assistantMessages = [assistantMessage];
      return {
        handled: true as const,
    ...
    if (
      googleConversationState.composeSession.session.status === "awaiting_body" &&
      looksLikeGoogleComposeSubjectOnlyUpdateText(params.text)
    ) {
      const nextSubject = await buildRevisedEmailSubjectOnly({
        instructionText: params.text,
        currentSubject: googleConversationState.composeSession.session.subject ?? "",
        currentBodyText: googleConversationState.composeSession.session.bodyPreview ?? "",
      });
      const updatedSession: GoogleComposeSession = {
        ...googleConversationState.composeSession.session,
        subject: nextSubject,
      };
      const assistantMessage = await createGoogleComposeSessionAssistantMessage({
        storage: params.storage,
        conversationId: params.conversationId,
        text: `Got it. I'll use ${JSON.stringify(nextSubject)} as the subject. What should the email say?`,
    ...
    params.onTrace?.("google.compose_session_retained_after_unmatched_turn", {
      sessionStatus: googleConversationState.composeSession.session.status,
      text: params.text,
      foregroundEmailTaskId: foregroundEmailTaskTarget?.taskId ?? null,
      latestEmailTaskId: latestEmailTaskTarget?.taskId ?? null,
    });
    const reminder = buildComposeSessionReminder(
      googleConversationState.composeSession.session,
    );
  ...
  if (googleConversationState.calendarSession) {
    const calendarSessionGoogleContext = buildGoogleTargetContextMetadata({
      connector: "calendar",
      action: "create",
      actionableTargetId:
        params.clientGoogleActionContext?.connector === "calendar"
    ...
    if (looksLikeGoogleClarificationConfusionText(params.text)) {
      ...
      const assistantMessage = await createGoogleCalendarSessionAssistantMessage({
        ...
        googleContext: calendarSessionGoogleContext,
      });
      const assistantMessages = [assistantMessage];
      return {
        handled: true as const,
  ...
  if (
    pendingEmailTaskTarget &&
    canImplicitlyResumePendingEmailTask &&
    !isGoogleActionApproveMessage(params.text) &&
    !isGoogleActionDeclineMessage(params.text) &&
    !isGoogleActionSendMessage(params.text) &&
    !isGoogleActionSaveAsDraftMessage(params.text) &&
    looksLikeGoogleEmailDraftRevisionInstruction(params.text)
  ) {
    const googleContext = buildGoogleTargetContextMetadata({
      connector: "gmail",
      action: "revise",
      actionableTargetId: pendingEmailTaskTarget.taskId,
      candidateTargetIds: [pendingEmailTaskTarget.taskId],
      sourceTurnId: params.userMessage.id,
      selectionReason: prefersPausedForegroundEmailTarget
        ? "paused_task"
        : "foreground_task",
      surfaceKey: params.clientGoogleActionContext?.surfaceKey ?? null,
      selectionMode: params.clientGoogleActionContext?.selectionMode ?? null,
    });
    params.onTrace?.("google.pending_task_resumed_for_revision", {
      connector: "gmail",
      taskId: pendingEmailTaskTarget.taskId,
    ...
    return handleResolvedGoogleEmailFollowUp({
      ...
      action: "revise",
      instructionText: params.text,
      targetTaskId: pendingEmailTaskTarget.taskId,
      targetPreview: pendingEmailTaskTarget.preview,
      googleContext,
      onEvent: params.onEvent,
    });
  }
  ...
  if (latestEmailTaskTarget && (wantsEmailSendFollowUp || wantsEmailSaveAsDraftFollowUp || wantsEmailRevisionFollowUp)) {
    const canUseForegroundEmailTarget =
      hasResolvableEmailForeground && !hasExplicitEmailTargetReference;
    if (
      actionableEmailCandidates.length > 1 &&
      emailCandidateResolution.kind !== "resolved" &&
      !explicitContextSelectedEmailTarget &&
      !canUseForegroundEmailTarget
    ) {
    ...
    const resolvedTarget =
      explicitContextSelectedEmailTarget ??
      activeContextSelectedEmailTarget ??
      (emailCandidateResolution.kind === "resolved"
        ? emailCandidateResolution.candidate
        : foregroundEmailTaskTarget ?? latestEmailTaskTarget);
    const resolvedSelectionReason =
      explicitContextSelectedEmailTarget
        ...
        : activeContextSelectedEmailTarget
          ? params.clientGoogleActionContext?.selectionReason === "paused_task"
            ? "paused_task"
            : params.clientGoogleActionContext?.selectionReason === "foreground_task"
              ? "foreground_task"
              : "active_surface"
        : emailCandidateResolution.kind === "resolved"
          ? "manual_selection"
          : canUseForegroundEmailTarget
            ? prefersPausedForegroundEmailTarget
              ? "paused_task"
              : "foreground_task"
  ...
  if (wantsCalendarFollowUp) {
    const canUseForegroundCalendarTarget =
      hasResolvableCalendarForeground && !hasExplicitCalendarTargetReference;
    if (
      rankedCalendarCandidates.length > 1 &&
      calendarCandidateResolution.kind !== "resolved" &&
      !explicitContextSelectedCalendarTarget &&
      !canUseForegroundCalendarTarget
    ) {
      const ambiguity: GoogleActionAmbiguityPrompt = {
        connector: "calendar",
        action: "update",
        ...
        candidates: rankedCalendarCandidates
          .slice(0, 3)
          .map((candidate, index) =>
            buildGoogleCalendarAmbiguityCandidateFromTarget(candidate, index),
          ),
      };
      params.onTrace?.("google.target_ambiguity_requested", {
        connector: "calendar",
      ...
      const assistantMessage = await createGoogleActionAmbiguityAssistantMessage({
        ...
        conversationId: params.conversationId,
        ambiguity,
        text: "I found a couple of recent calendar events. Which one should I update?",
        googleContext: buildGoogleTargetContextMetadata({
          connector: "calendar",
          action: "update",
          candidateTargetIds: ambiguity.candidates.map((candidate) => candidate.taskId),
      ...
      return {
        ...
        assistantMessages,
        legacyAssistantMessage: makeLegacyAssistantMessage(assistantMessages),
        model: "google_action_calendar_ambiguity_v1",
        decisionPath: "companion_reply" as const,
        decisionPathReason: "companion" as const,
        awaitingApproval: false,
        task: null,
    ...
    const resolvedCalendarTarget =
      ...
      (calendarCandidateResolution.kind === "resolved"
        ? calendarCandidateResolution.candidate
        : foregroundCalendarTaskTarget ?? rankedCalendarCandidates[0] ?? null);
    if (resolvedCalendarTarget) {
      if (
        ...
        params.onTrace?.("google.target_context_mismatch", {
          ...
          action: "update",
          activeTargetId: params.clientGoogleActionContext.actionableTargetId,
          resolvedTargetId: resolvedCalendarTarget.taskId,
          resolutionKind: calendarCandidateResolution.kind,
        });
      }
      const googleContext = buildGoogleTargetContextMetadata({
        connector: "calendar",
        action: "update",
        actionableTargetId: resolvedCalendarTarget.taskId,
        candidateTargetIds: rankedCalendarCandidates.map((candidate) => candidate.taskId),
        sourceTurnId: params.userMessage.id,
        selectionReason:
          explicitContextSelectedCalendarTarget
      ...
      params.onTrace?.("google.target_resolved", {
        connector: "calendar",
        action: "update",
        selectedTaskId: resolvedCalendarTarget.taskId,
        selectionReason: googleContext.selectionReason,
      });
      const preparation = await prepareGoogleActionTask({
        storage: params.storage,
        ...
        recentContext: {
          ...recentContext,
          recentCalendarTask: buildRecentGoogleActionTaskFromTarget(
            resolvedCalendarTarget,
          ),
        },
      });
      return finalizePreparedGoogleActionTask({
  ...
  if (googleConversationState.pendingTask) {
    ...
    const wantsDraftRevision =
      ...
      !wantsDecline &&
      !startsFreshGoogleActionRequest &&
      googleConversationState.pendingTask.preview?.connector === "gmail" &&
      googleConversationState.pendingTask.preview.proposedEmail &&
      (googleConversationState.pendingTask.preview.kind === "email_compose" ||
        googleConversationState.pendingTask.preview.kind === "email_reply") &&
      looksLikeGoogleEmailDraftRevisionInstruction(params.text);
    ...
    if (wantsApprove || wantsDecline) {
      ...
      if (wantsDecline) {
        ...
        await params.storage.createMessage({
          conversationId: task.conversationId,
          sender: "assistant",
          text: "Understood. I canceled that Google action.",
          partIndex: 0,
          uiPayload: {
            kind: "agent_task_status",
            task: toAgentTaskSummary(cancelled),
            text: "Canceled",
            googleActionPreview: googleConversationState.pendingTask.preview,
          },
        });
        ...
        return {
          ...
          assistantMessages,
          legacyAssistantMessage: makeLegacyAssistantMessage(assistantMessages),
          model: "google_action_task_cancelled_v1",
          decisionPath: "agent_task" as const,
          decisionPathReason: "task_started" as const,
          awaitingApproval: false,
          task: toAgentTaskSummary(cancelled),
      ...
      return {
        ...
        model: wantsSend
          ? "google_action_task_send_v1"
          : "google_action_task_approved_v1",
        decisionPath: "agent_task" as const,
        decisionPathReason: "task_started" as const,
        awaitingApproval: false,
        task: approvedTask,
    ...
    if (wantsDraftRevision) {
      const googleContext = buildGoogleTargetContextMetadata({
        connector: "gmail",
        action: "revise",
        actionableTargetId: googleConversationState.pendingTask.taskId,
        candidateTargetIds: [googleConversationState.pendingTask.taskId],
        sourceTurnId: params.userMessage.id,
        selectionReason: prefersPausedForegroundEmailTarget
          ? "paused_task"
          : "foreground_task",
  ...
  return finalizePreparedGoogleActionTask({
    ...
    googleContext:
      preparation.kind === "ready"
        ? buildGoogleTargetContextMetadata({
            connector: preparation.preview.connector,
            action:
              preparation.preview.connector === "gmail"
                ? preparation.preview.proposedEmail?.sendAfterApproval
                  ? "send"
                  : "revise"
                : preparation.preview.kind === "calendar_update"
                  ? "update"
                  : "create",
            actionableTargetId: params.clientGoogleActionContext?.actionableTargetId ?? null,
            sourceTurnId: params.userMessage.id,
            selectionReason: params.clientGoogleActionContext?.selectionReason ?? "latest_actionable",
            surfaceKey: params.clientGoogleActionContext?.surfaceKey ?? null,
            selectionMode: params.clientGoogleActionContext?.selectionMode ?? null,
          })
        : params.clientGoogleActionContext
          ? buildGoogleTargetContextMetadata({
              ...
              sourceTurnId: params.clientGoogleActionContext.sourceTurnId ?? null,
              selectionReason:
                params.clientGoogleActionContext.selectionReason ?? "latest_actionable",
              surfaceKey: params.clientGoogleActionContext.surfaceKey ?? null,
              selectionMode: params.clientGoogleActionContext.selectionMode ?? null,
            })
          : null,
...
async function buildModelMessages(params: {
  ...
  const safeConversationMemory = filteredMemory
    ...
    .filter((message) => !isGoogleConnectionFailureMessage(message));
  const currentConversationMemory = safeConversationMemory.filter(
    (message) =>
      !(
        params.suppressStructuredGoogleActionMemory &&
        shouldExcludeMessageFromCompanionCurrentThreadDuringGooglePause({
          message,
          googleConversationState: params.googleConversationState ?? null,
        })
      ),
  );
  ...
  const modelMessages = currentConversationMemory.map((message) => ({
    sender: message.sender,
    text: message.text,
    attachments: message.attachments
      .filter((attachment) => attachment.status !== "deleted")
...
export async function registerRoutes(
        ...
        for (const functionCall of parsed.functionCalls) {
          ...
          if (
            ...
            trace(req, "live.tool.google_action.context", {
              ...
              hasActionAmbiguity: Boolean(googleConversationState.actionAmbiguity),
              ambiguityConnector:
                googleConversationState.actionAmbiguity?.prompt.connector ?? null,
              ambiguityCandidateCount:
                googleConversationState.actionAmbiguity?.prompt.candidates.length ??
                0,
              ...
              calendarEventCandidateCount:
                googleConversationState.calendarEventCandidates.length,
              googleActionContext: parsed.googleActionContext ?? null,
            });
  ...
  app.post("/api/chat/respond", isAuthenticated, async (req: any, res) => {
    ...
    try {
      ...
      const suppressStructuredGoogleActionMemory =
        shouldSuppressStructuredGoogleActionMemoryForCompanionTurn({
          text: parsed.text,
          state: googleFallbackState,
        });
      ...
      const modelMessages = await buildModelMessages({
        ...
        mediaStore,
        suppressStructuredGoogleActionMemory,
        googleConversationState: googleFallbackState,
      });
