...
## Recent Changes
- 2026-03-12: Voice mic sensitivity & adaptive detection — replaced hardcoded speech detection threshold floor (0.008) with adaptive calibration system that measures actual mic ambient RMS during first ~500ms of audio frames and scales the floor to `max(0.001, ambientRms * 6)`, capped at the original 0.008; fixes the fundamental issue where quiet mics (RMS ~0.00007) couldn't be detected against a 0.008 floor (100x gap); adaptive floor applied to all threshold calculations including idle base, assistant base, candidate hysteresis min, and mobile barge-in peak/avg RMS; added `voiceIsolation: true` to all mic constraint sets (desktop + mobile); enabled `autoGainControl: true` on mobile `mobile_voice_safe` profile (was `false`); increased audio prefix buffer from 8→14 frames to capture more speech onset; added `<noise>`/`<music>`/`<silence>` etc. non-speech transcript tag filter (both client and server); mic gain level diagnostic logging (`very_low`/`low`/`normal`/`high`) in debug state and calibration trace events
- 2026-03-12: Semantic memory system — replaces loading all messages per session with tiered retrieval: (1) recent raw messages via SQL LIMIT (last ~50), (2) semantically searched conversation summaries via `gemini-embedding-001` embeddings (256 dims, cosine similarity, 0.3 threshold), (3) durable memory items with embedding-based ranking, (4) keyword fallback for items without embeddings, (5) 12KB context safety cap. New `server/memory.ts` module handles embedding generation, vector search, conversation block summarization (every 50 messages via `gemini-2.0-flash-lite`), and backfill. Schema additions: `embedding TEXT` on `user_memory_items`, `summary` kind in `memoryItemKindEnum`, `messageCount`/`lastSummarizedIndex`/`lastSummarizedAt` on `conversations`. Profile facts and long-term memory placed before recent thread in context for priority. Embeddings auto-recomputed on memory item updates.
...
- `server/memory.ts` — Semantic memory: embeddings (gemini-embedding-001), vector search, conversation summarization, backfill
