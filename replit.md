...
## Recent Changes
- 2026-03-06: Fixed voice transcription showing wrong language — root cause: `speechConfig` in Live API token had no `languageCode`, causing auto-detection to misidentify English speech as French/Arabic/etc; fix: added `languageCode: "en-US"` to `speechConfig` (configurable via `GEMINI_LIVE_SPEECH_LANGUAGE_CODE` env var)
