...
## Recent Changes
- 2026-03-06: Fixed voice transcription showing wrong language — root cause: per Google docs, native audio models "automatically choose the appropriate language and don't support explicitly setting the language code" in `speechConfig`; the input transcription service auto-detects language independently; fix: (1) removed `languageCode` from `speechConfig` (not supported by native audio models, may interfere), (2) added "LANGUAGE AND TRANSCRIPTION POLICY" section to live system instruction guiding the model to rely on what it HEARS in audio rather than potentially garbled transcription text, respond in whatever language the user speaks, and default to English
